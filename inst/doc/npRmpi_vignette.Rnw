...
\subsection{Parallel R Terminal Execution}

Often you might wish to run your code instead in an R
terminal/console. Doing so requires the essential elements in Section
\ref{sec example} below with two minor differences. First, you do not
require the \code{.Rprofile} to reside in the root or current
directory, and second you will load the \pkg{npRmpi} pacakage and then
generate the slave nodes using the command
\code{mpi.spawn.Rslaves(nslaves=foo)} where on a dual core desktop
\code{foo=1} so that \code{mpi.spawn.Rslaves(nslaves=1)} will spawn
one slave node in addition to the master node on which \proglang{R} is
running. See the file \code{interactive_Rterm.R} in the demo directory
for an illustration, but here are the additional elements required in
addition to those below given the caveats above.
