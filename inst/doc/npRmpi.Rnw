...
npindex (Ichimura) & 18.1 & 35.0 & 0.52\cr
npindex (Klein/Spady) & 27.2 & 61.7 & 0.44\cr
...
\item The function \code{npunitest} currently runs orders of magnitude
  slower under \proglang{MPI} when using \code{method="integration"}
  (default). This appears to be related to a limit on the number of
  nested function calls in \proglang{R}. For the time being you are
  advised to use the serial \pkg{np} package if you require this
  function or select \code{method="summation"} (see \code{?npunitest}
  for caveats, though this function is quite efficient computationally
  speaking and can handle quite large datasets by default via the
