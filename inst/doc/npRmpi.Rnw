...
\item The functions \code{npunitest} and \code{npdeptest} currently
  ...
  \code{method="integration"} (default). This appears to be related to
  a limit on the number of nested function calls in \proglang{R}. For
  the time being you are advised to use the serial \pkg{np} package if
  you require this function or select \code{method="summation"} (see
  \code{?npunitest} and \code{?npdeptest} for caveats, though these
  functions are quite efficient computationally speaking and can
  handle quite large datasets by default via the serial
  implementation).
