...
run, and finally you will run the job using either \code{mpirun} or,
...
batch queuing systems on your local cluster).
...
downloading the \pkg{npRmpi} tarball and, from a command shell,
running
...
directory and again name it \code{.Rprofile}). Then to run the job
using two processors on an Open \proglang{MPI} system you will enter
something like
