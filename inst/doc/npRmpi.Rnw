...
\item The function \code{npunitest} currently runs orders of magnitude
  ...
  (default). For the time being you are advised to use the serial
  \pkg{np} package if you require this function or select
  \code{method="summation"} (see \code{?npunitest} for caveats, though
  this function is quite efficient computationally speaking and can
