...
\item The \code{C} code underlying regression cross-validation
  ...
  correct with the latter being a tad slower (one one processor that
  is - see the timings above for a comparison). Note that, due to the
